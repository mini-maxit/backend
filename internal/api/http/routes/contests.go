...
func (cr *ContestRouteImpl) GetMyContestResults(w http.ResponseWriter, r *http.Request) {
	...
	results, err := cr.contestService.GetMyContestResults(tx, currentUser, contestID)
	if err != nil {
		db.Rollback()
		status := http.StatusInternalServerError
		switch {
		...
		case errors.Is(err, myerrors.ErrNotAuthorized):
			status = http.StatusForbidden
		default:
			cr.logger.Errorw("Failed to get contest results", "error", err)
		}

		httputils.ReturnError(w, status, "Failed to get contest results")
		return
	}
